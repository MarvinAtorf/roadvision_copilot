...
def resolve_canonical_id(
    ...
    # Unlike vehicle/traffic-light tracking, candidates are NOT filtered by
    # class_id here: the same physical sign can be classified differently
    # from frame to frame (e.g. "60" misread as "40" from a distance), and
    # SignAnalyzer's confidence-vote only works if the spatial track
    # survives across those disagreements instead of splitting into a new
    # canonical_id every time the raw classification flips.
